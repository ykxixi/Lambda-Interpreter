...
public class Interpreter {
        ...
        while(true){
            if(isApplication(ast)){
                if(isAbstraction(((Application)ast).getLhs())){
                    ...
                    if(!isIdentifier(((Application) ast).getRhs())) {
                        ((Application) ast).setRhs(evalAST(((Application) ast).getRhs()));
                    }
                    //只要仍是Application或body可继续运算的Abstraction，就一直递归下去，最终得到body不能再运算的Abstraction或Identifier
                    if(isAbstraction(((Application) ast).getLhs())){
                        ast=evalAST(ast);
                    }
                    return ast;
    ...
    private AST subst(AST node, AST value, int depth){
        ...
        if(isApplication(node)){
            return new Application(
                    subst(((Application) node).getLhs(),value,depth),
                    subst(((Application) node).getRhs(),value,depth)
            );
        }
        //(\.\.\.(1 ((2 1) 0)) \.\.0)
        //每进一层Abstraction深度+1
            ...
            return new Abstraction(
                    ((Abstraction) node).getParam(),
                    subst(((Abstraction) node).getBody(),value,depth+1)
            );
        }
        else{
    ...
    private AST shift(int by, AST node,int from){
        ...
        if(isApplication(node)){
            AST lhl = shift(by,((Application)node).getLhs(),from);
            AST rhl = shift(by,((Application) node).getRhs(),from);
            return new Application(lhl, rhl);
        }
        //(\.\.\.(1 ((2 1) 0)) \.\.0)
        //每进一层Abstraction深度+1
        else if(isAbstraction(node)){
            Identifier identifier = ((Abstraction)node).getParam();
            AST body = shift(by,((Abstraction) node).getBody(),from + 1);
            return new Abstraction(identifier,body);
        }
        else{
            String name = ((Identifier)node).getName();
            String value= String.valueOf(((Identifier)node).getDBindex() +
                    (((Identifier) node).getDBindex()>=from?by:0));
